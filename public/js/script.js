document.getElementById('badge-form').addEventListener('submit', function(event) {
    ...
    fetch('/Gafetes/save-form', { // Cambia aquí
        method: 'POST',
        body: formData
    })
    .then(response => {
    ...
    .then(data => {
        ...
        fetch(`/Gafetes/datosQR/${badgeId}`) // Cambia aquí también
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                designImg.src = '/Gafetes/img/Gafete.png'; // Usa la ruta relativa adecuada
            })
            .catch(error => {
